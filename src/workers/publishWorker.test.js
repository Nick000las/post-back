...
describe('publishWorker.processarJob', () => {
    ...
    test('lança UnrecoverableError se o post não existe (não tenta adapter nenhum)', async () => {
        ...
        const job = criarJobFake({ postId: 1, accountId: 1, userId: 1 });
    ...
    test('busca a conta já filtrando pelo userId do job (não vaza conta de outro usuário)', async () => {
        prismaAdapter.buscarPostPorId.mockResolvedValue({ id: 1 });
        ...
        const job = criarJobFake({ postId: 1, accountId: 2, userId: 77 });
    ...
    test('publica no instagram com sucesso, descriptografa o token e finaliza o status do post', async () => {
        const post = { id: 1, caption: 'x', file_path: 'a.jpg', file_type: 'image/jpeg' };
        ...
        const job = criarJobFake({ postId: 1, accountId: 2, userId: 9 });
        await processarJob(job);
    ...
    test.each([
        ...
        const post = { id: 1, file_path: 'a', file_type: 'image/jpeg' };
        ...
        const job = criarJobFake({ postId: 1, accountId: 2, userId: 1 });
        await processarJob(job);
    ...
    test('erro de negócio do adapter (AppError): fecha o resultado imediatamente, não espera retry', async () => {
        ...
        const job = criarJobFake({ postId: 1, accountId: 2, userId: 1 }, { attemptsMade: 0, opts: { attempts: 3 } });
    ...
    test('erro transiente ANTES da última tentativa: não grava FAILED nem finaliza, só relança pro BullMQ tentar de novo', async () => {
        const post = { id: 1, file_path: 'a', file_type: 'image/jpeg' };
        ...
        // attemptsMade=0 de attempts=3 -> essa é a 1ª tentativa, ainda restam mais 2
        const job = criarJobFake({ postId: 1, accountId: 2, userId: 1 }, { attemptsMade: 0, opts: { attempts: 3 } });
    ...
    test('erro transiente NA última tentativa: grava FAILED e finaliza antes de relançar', async () => {
        ...
        // attemptsMade=2 de attempts=3 -> essa é a 3ª e última tentativa
        const job = criarJobFake({ postId: 1, accountId: 2, userId: 1 }, { attemptsMade: 2, opts: { attempts: 3 } });
